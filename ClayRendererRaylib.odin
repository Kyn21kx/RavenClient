...
raylib_measure_text :: proc "c" (
	...
	// Unused in calculation, but tracked in original C code
	// max_line_char_count: int = 0

	fonts := cast([^]rl.Font)user_data
	font_to_use := fonts[config.fontId]

	// Font failed to load? Fallback to default.
	if font_to_use.glyphs == nil {
		font_to_use = rl.GetFontDefault()
	}

	scale_factor := f32(config.fontSize) / f32(font_to_use.baseSize)

	for i in 0 ..< text.length {
		// Assuming text.chars is a pointer ([^]u8 or cstring)
		char := text.chars[i]

		if char == '\n' {
			max_text_width = math.max(max_text_width, line_text_width)
		...
		// Safety check: ensure index is within bounds if font glyphs are limited
	...
	text_size.width =
		max_text_width * scale_factor + (f32(line_char_count) * f32(config.letterSpacing))
	text_size.height = f32(config.fontSize)
...
measure_text_ascii :: proc "c" (
	...
	// Use Raylib's MeasureTextEx which handles all edge cases
